...
See `agents/claude/ts-profile-setup/SKILL.md` (Add section) for the exact pattern.
...
Full pattern: `agents/claude/ts-profile-setup/SKILL.md` (Derive names section).
...
1. Follow the same Keychain + env var pattern (see ts-profile-setup for reference)
