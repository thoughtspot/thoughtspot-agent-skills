...
def test_allow_published_overrides_the_refusal(tmp_path, planning_client):
    # msg_runner, not runner: the warning that the rule will not reach any tenant Org
    # is printed to stderr, and only msg_runner's mixed output can see it. With
    # `runner` this assertion would be silently unreachable, so deleting the warning
    # print would not fail any test.
    client = planning_client(FakeClient({UPDATE: FakeResponse(None, 204)}))
    plan = _plan(blocked="CSR_BLOCKED: 'T2' is published")
    result = msg_runner.invoke(app, ["security", "column-rules", "apply", "--input",
    ...
    assert "does NOT travel with publication" in result.output
