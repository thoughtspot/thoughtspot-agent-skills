...
def _propagate_rls_or_fail_closed(cand: dict, plans: dict, model_tml: dict,
    ...
    primary_patched.tml.yaml) — if the candidate's grain still omits a
    required RLS filter column: an aggregate that can't be secured must never
    be emitted, not even partially.

    Returns `{}` (no-op) when no base Table TML in `table_tmls` carries
    `rls_rules` at all.
...
def generate(
    ...
    # call (SpotQL/primary export) is made — an unsecurable candidate must
    # exit with zero side effects, not a partial set of artifacts.
