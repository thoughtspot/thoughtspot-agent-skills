...
ts_cli/
  ...
  migrate/apply_plan.py -- Phase 2 `ts migrate apply` planning engine: step-0 validation (validate_apply/find_rename_collisions -- the collision check exists because the rename map is GENERATED and its failure is SILENT: a rename cascades to every dependent automatically, so a wrong target quietly repoints real content at the wrong column rather than erroring), connection handling (connection_action -- names are per-Org, so a same-named connection in the target lets lifted TML resolve UNCHANGED; a mismatch falls back to rewriting one field per Table rather than failing), the ordered plan (build_apply_plan/build_cleanup_steps -- cleanup runs Models then Tables then connection because connection deletion does NOT cascade, and a Model that still has dependents REFUSES to delete, which is the missed-repoint check rather than an error to force past), and the resume ledger (pending_steps keys on step NAME not index, so a plan regenerated after a mapping edit still skips completed work) (pure functions, no I/O)
  migrate/apply_exec.py -- Phase 2 executor: the I/O half of `ts migrate apply`, one run_* per step over a Ctx carrying both clients plus the live ledger. Holds the BL-144 guard (_assert_rls_survived -- a malformed rls_rules block imports with status_code OK, is discarded, AND destroys the rule already on the table, so `OK` is never sufficient evidence on a write to a table that carried RLS), reuses dependency/mutate.apply_repoint rather than reimplementing search_query sanitisation, and reads each imported document's own top-level key to tell Tables from Models -- both are LOGICAL_TABLE to the API and cleanup must delete Models first
...
Current version: **0.115.0**. Run `python tools/validate/check_version_sync.py` to verify.
