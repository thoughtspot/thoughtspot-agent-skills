...
ts_cli/
  ...
  commands/
    ...
    security.py   -- ts security column-rules (get, export, set, clear) -- Column Security Rules (CSR), the other column-security mechanism, and not interchangeable with `ts share`'s column-level sharing (CLS): CSR cannot be defined on a published object, declares only the RESTRICTED columns rather than every visible one, and leaves liveboard filters on secured columns interactive. Holds the group's shared substrate -- _fetch_rules, _post_update, _published_orgs (reads metadata_header.orgIds, the same field `ts publish status` reads, so no per-Org auth is needed), and _fail (turns the feature-flag 403/10023 into an explained failure rather than a bare "Forbidden") -- while reusing _client_for_org / assert_org_context / _resolve_object from share.py rather than reimplementing them, because groups are per-Org and an ambiguous name is exactly as dangerous to resolve wrong here as it is for sharing
    security_planning.py -- ts security column-rules resolve / apply / build / import (attaches to security.column_rules_app); the manifest layer (TS_COLUMN_SECURITY_RULES, --init-table, --source uniform|file|db) mirroring share_planning.py, plus the CSR_BLOCKED refusal -- a published table cannot carry column security rules, re-checked in both build and apply (only apply's --allow-published can override it) because a plan is a file a human can edit in between. Split from security.py under the file-size gate
