...
1. **JSON Schema validation** (`validate_schema`, `validate.py:78-85`) — validates the raw parsed YAML dict against `core-spec/ossie-schema.json` (Draft 2020-12) using `jsonschema.Draft202012Validator`. This is the authoritative structural gate: required fields, enum membership (`Dialect`, `DataType`), and `additionalProperties: false` on every `$defs` object (so unknown/extra keys anywhere in the document fail validation).
...
> **F18 CLOSED upstream, 2026-07-31** (commit `88e0011`, PR #261 "CI for databricks converter"). Both `.github/workflows/converter-databricks-ci.yml` and `converters/databricks/uv.lock` exist at `b5da5d6`; ten of the eleven converters now carry a CI workflow. The finding's *lesson* survives — a test suite nothing invokes is not a gate, which is the same shape as this repo's own angle-9 miss — but the instance is resolved, and it should no longer be cited as live evidence.
...
on this branch, not from this report: the `ts-ossie-function-mapping.md` review surfaced four
