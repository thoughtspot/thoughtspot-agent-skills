...
**Impact — this changes numbers, silently.** Every fact row whose FK does not match a dimension row is retained by the Databricks MV and dropped by the ThoughtSpot Model. TPC-DS `store_sales` deliberately generates NULL foreign keys, so `total_sales` and `total_quantity` will read **lower** in ThoughtSpot than in Databricks on the same data, with no warning anywhere in the pipeline. The round-trip diff cannot see it (the MV has no `type:` to compare), `ts tml lint` cannot see it, and `check_tml.py` cannot see it.
...
- **No numeric verification.** F1's impact (dropped fact rows) is reasoned from the vendor's documented join semantics and TPC-DS's nullable FKs, not measured. Confirming it needs a live workspace and instance with the same data on both sides — the parked conversion-fidelity angle (audit angle 15).
