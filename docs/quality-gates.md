...
| 22 | `skill context cost` | check_skill_context_cost.py — gate the per-invocation context cost of skills. | Skill files staged (pre-commit, CI) | Skill context cost — a SKILL.md is loaded into context on every invocation; gate its estimated-token size (warn >12k, fail >25k — BL-128 extraction is | gate | unknown |
