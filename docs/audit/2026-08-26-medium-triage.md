...
| **18.1** | ✅ **CLOSED 2026-08-27.** Allow-list widened in the **committed** `.claude/settings.json` (it is shared repo state, so every contributor hit the same wall): `WebSearch`, seven domain-scoped `WebFetch` rules, and the two **read-only** SpotterCode tools; `execute-thoughtspot-code` is **denied**, not merely unlisted. Closes **18.3** too — `docs.claude.com` / `docs.anthropic.com` only redirect cross-host to `platform.claude.com`, which WebFetch does not follow, so the first cut granted angle 18 nothing on the page it exists to read. `check_audit_workflow_permissions.py` keeps it from rotting — adding a platform to `PLATFORMS` now fails the gate until its docs domain is approved, instead of blocking that one finder mid-sweep. Original blocker: The audit workflow's own premise is broken — every finder is told to use WebSearch/WebFetch/SpotterCode, none of which is in `permissions.allow`, so a 7- or 14-agent sweep blocks on interactive prompts. The fix is a **shared** allow-list widening (web domains + MCP tools), which is a policy call rather than a mechanical edit. |
