...
> **One row never closed.** Fifteen of sixteen bucket-A rows are ✅ across PRs #465–#472.
> **13.14 / 13.15** is not — verified 2026-08-27, the three target files are unchanged —
> and it is now **BL-224** rather than an unmarked row in a triage document.
...
| **13.14 / 13.15** | ⚠️ **NOT CLOSED — FILED (BL-224) 2026-08-27.** The only bucket-A row that exited neither as a fix nor a deflection; `mv_emit_joins.py` is untouched since PR #426 and both mapping files still say "equivalent"/"never both". `rely`/`cardinality` are neither equivalent nor exclusive — the vendor *recommends both* where the constraint holds, so every one-to-many join drops a recommended optimization. 13.15's one-to-many restrictions are import-time failures, therefore catchable. | doc + emitter change + a deterministic pre-emit check |
