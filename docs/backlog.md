...
| BL-297 | 27 of 51 ts-audit checks are imported by a test but never CALLED by one — import satisfies a coverage grep and exercises nothing, which is how `check_p6`/`check_d2` shipped structurally unable to return a finding. Each needs a fixture that TRIPS it, and proving a check can fire is per-check domain work, not a sweep | next ts-audit pass |
...
| BL-299 | **`index_type` sense is inverted in three checks** — S2 (`checks_security.py:66`), P9 (`checks_perf.py:224`), P11 (`:242`) all treat *presence* of `index_type` as "indexed", but the schema says omit-for-default-indexed. So the real risk (PII indexed by default) is invisible and `DONT_INDEX` (indexing suppressed) false-positives. The only value this repo ever writes is `DONT_INDEX`, so every converter-produced model turns S2 into a pure false-positive generator | next ts-audit pass |
| BL-300 | **`check_p17` cannot fire on any model the audit can see.** It matches formula cross-references by display name; real TML refs are `formula_<name>` ids, and a display-name ref fails on first import — so no exported model carries the shape it looks for. Verified: 0 findings on a real id-ref chain | next ts-audit pass |
| BL-301 | **`check_h7` compares two disjoint GUID namespaces and is inverted.** `model_tables[].fqn` is a Table guid; `answer.tables[].fqn` is a Model guid. Every healthy answer is flagged "bypassing the model layer", and the direct-table answer it exists to find is silently excused | next ts-audit pass |
| BL-302 | **`check_h5` asserts a lookup it never makes.** `ctx.dependents` never holds a SET guid — `build_context` only queries dependents for models and tables — so `if not set_deps` is always true and every set is reported an orphan, with a detail claiming zero consumers were found | next ts-audit pass |
| BL-303 | **`check_p5`'s date-constraint suppression is dead on real TML.** `constraints` exports as a mapping, so `for c in constraints` yields the key string `"constraint"`, which never contains `date_range_condition` — a model with a genuine rolling window is still reported as having none. The existing test helper passes a list, which is the shape that hides it | next ts-audit pass |
| BL-304 | **the data/perf check split was made by copying, not extracting**: `d4`≡`p4` and `s9`≡`p14` are verbatim, `s8` ⊇ `p15`, `_join_depth`≡`p7`, `_table_role` is written twice, `d1`'s column rule ≡ `p8`. Each pair double-counts the same problem in one report under two angles, and a threshold change fixes one and not the other — the same shape as the `d2`/`p6` pair fixed in #528 | next ts-audit pass |
...
**Status:** RESOLVED 2026-07-30 (ts-cli v0.126.1; five emitters fixed, qlik map audited
end-to-end, 84 live probes on se-thoughtspot, nothing persisted).

### How it was resolved -- one line per module

| Module | Fix | Skill |
|---|---|---|
| `sv_sql.py` | `TRIM`/`LTRIM`/`RTRIM` moved into `_PASS_THROUGH_HINT` (the `UPPER`/`LOWER` path); `REPLACE`/`STARTSWITH`/`ENDSWITH` into a new `_STRING_COMPOSED` dispatch emitting the doc's pass-through and `strpos`/`substr` compositions | from-snowflake-sv 1.19.5 |
| `databricks/mv_sql.py` | same two mechanisms; **`ENDSWITH` gained a mapping it never had** (it was unmapped, not mis-emitted) | from-databricks-mv 1.11.0 |
| `tableau/functions.py` | the `TRIM(` -> `trim ( ` regex rewrite deleted; `TRIM` plus **new** `LTRIM`/`RTRIM` handlers added to `_ARG_HANDLERS` beside `UPPER`/`LOWER` | from-tableau 1.40.0 |
| `qlik/functions.py` | six defect classes fixed (below), `FUNCTION_MAP` audited entry-by-entry, `PASSTHROUGH_MAP` added, and the **stale packaged JSON** brought back into step with the shared markdown | from-qlik 1.0.2 |
| `powerbi/functions.py` | `TRIM`/`UPPER`/`LOWER` -> pass-throughs; `HOUR` -> `hour_of_day`; `DISTINCTCOUNT` -> `unique count` (space); `MONTH` -> `month_number` (`month` returns the NAME); `MINUTE`/`SECOND` removed so they flag rather than fake | from-powerbi 1.1.1 |
| `formula_common.py` | new shared `wrap_passthrough_calls(text, templates, quote)` -- one quote/paren-aware rewriter for the two regex-pipeline converters (qlik, powerbi), replacing qlik's private `_wrap_string_ops`. The two parser-based converters keep their own `_call_pass_through` | — |

**The qlik map was worse than this entry predicted -- six classes, not four.** The two extra:

5. **`unique_count` (underscore) does not exist.** `translate()` emitted it for
   `Count(DISTINCT x)`; the function is `unique count`, with a space. Only the conditional
   variants carry an underscore (`unique_count_if`). Live-disproved 2026-07-30. The same bug
   was in `powerbi/functions.py` and in the powerbi mapping doc.
6. **Qlik `Concat()` is an AGGREGATION.** It joins values *across* rows (like `GROUP_CONCAT`);
   ThoughtSpot `concat()` joins within one row. It was name-mapped, so it produced a
   *valid-but-wrong* formula -- the one failure mode the module's own flag-don't-downgrade
   contract exists to prevent, and the only defect here that a probe can never catch. Now
   mapped to `None` (flagged NEEDS REVIEW). Row S14 had documented the mismatch all along.

Two corrections to this entry's own text, for the record:

- **`upper`/`lower` in qlik were never actually broken.** The entry read `FUNCTION_MAP` and
  concluded they were emitted bare, but a post-pass (`_wrap_string_ops`) already rewrote them
  into `sql_string_op`. The *map value* was a non-existent name; the *emitted formula* was
  correct. That is why the fix keeps the marker-plus-post-pass architecture and adds a test
  asserting the marker never survives, rather than restructuring it.
- **No vendored-lib commit was needed.** `agents/databricks/notebooks/databricks_mv_lib.py` is
  gitignored (`.gitignore:63`) and `test_vendor_mv_lib.py` rebuilds it from source in-process,
  so the sync it enforces is automatic. It was regenerated locally anyway; nothing to commit.

**Live verification (se-thoughtspot, 2026-07-30, `ts tml import --policy VALIDATE_ONLY`).**
84 probes across three waves on the Payroll Test Model
(`acf62370-9744-4178-a7c5-1b3ba35dc930`), one throwaway formula per probe. Controls behaved:
`sum ( )` accepted, `bogus_fn ( )` rejected. Wave 3 is the decisive one -- its 30 probe inputs
are the **verbatim output of the fixed emitters**, generated by calling them with real Payroll
column names, and **30/30 validated**. Newly disproved by name: `len`, `mid`, `ceiling`,
`power`, `log`, `day_of_month`, `date_trunc_month`/`_year`/`_quarter`/`_week`, `quarter`,
`hour`, `minute`, `second`, `unique_count`. Newly *confirmed to exist*: `exp`, `sign`,
`to_bool`, `to_date`, `group_sum`, `group_unique_count`, `unique_count_if`, `date`,
`hour_of_day`, `quarter_number`, `month`, `start_of_month`. Also settled two emission-style
questions the qlik/powerbi modules depended on: an **unspaced** call (`sum(x)`) and a
**single-quoted** `sql_string_op` template both import, so neither module needed restyling.
Zero persistence confirmed twice: `ts metadata search --name "%BLPROBE%"` -> `[]`, and the
Model re-exported byte-identical (9 formulas / 8 columns, no `BLPROBE` anywhere).
...
**Validator promotion, restated (2026-07-30 — still OPEN, and now better specified).** The fix
above installed *test-level* guards, not repo-level ones:

- `test_qlik_functions.py::TestMapIntegrity` asserts every `FUNCTION_MAP` value is a verified
  ThoughtSpot function, `None`, or a `PASSTHROUGH_MAP` marker — but only for qlik, and against
  a catalog set **hardcoded in the test file**, which is itself a copy that can drift.
- `TestSharedReferenceSync` asserts the packaged qlik JSON matches the shared markdown — the
  exact drift BL-170 left behind — but again, qlik only.
- Per-module sweeps assert no bare disproved name is ever emitted (sv_sql, mv_sql, tableau,
  qlik, powerbi).

The promotion is therefore unchanged in intent and sharper in scope: teach
`check_formula_catalog.py` to (a) parse `_RENAME` / `_DAX_FUNC` / `FUNCTION_MAP` /
`_PASS_THROUGH_HINT` / `PASSTHROUGH_MAP` out of `tools/ts-cli/ts_cli/**` and fail on any value
the catalog strikes through, and (b) generalise the qlik markdown↔JSON sync check to any
mapping that ships a machine-readable twin. That would replace the hardcoded catalog copy in
the test with the real one and extend both guarantees to the other four modules. **Blocked on
BL-172** (the scanner silently skips 82% of the qlik file's rows), exactly as BL-170 said.

**Resolution target:** was "next converter-formula pass" — done 2026-07-30, same day the
follow-on was filed. Tier 1 was right: it was producing failed imports on five of the six
conversion paths.
...
## BL-191 -- `dependency/mutate.py` reads Views through `column_id`, a field real View TML does not have `Tier 1` -- **RESOLVED 2026-07-31**
...
**Status:** **RESOLVED 2026-07-31** -- fixed in ts-cli **v0.127.1**
...
2. **`_strip_view_formulas`' second half was dead** -- a removed formula's **display
   name** is now passed back through `_view_column_targeted`, so the surfacing column
   goes with it. Measured over all 42 real Views: the old `column_id == formulas[].id`
   binding bound **0 of 46** formulas, and replaying a formula-triggering strip left
   **101 dangling references** where the fixed code leaves **0**.
3. **Docstrings asserted the disproved shape** -- both rewritten from the census
   (`docs/reviews/2026-07-30-tml-census.md`, cited by tracked path); the `::`-prefix
   claim is gone. `agents/cli/ts-dependency-manager/SKILL.md`'s troubleshooting row and
   `references/dependency-types.md`'s View row + new note were carrying the same claim
...
6. **`search_query` still named the cascaded-away formula.** `_strip_view_search_query`
   ...
   id. `OptumRx View`'s real query begins `[formula_PMPM month] [formula_PMPM month].monthly`;
   removing `Fill Date` dropped that formula and left the query naming it, the same
   dangling reference by another route. `_strip_view_formulas` now re-sanitizes against the
   removed formulas' ids and names. Corpus-measured: **94 cases** with the old code, **0**
   with the fix. `sanitize_search_query` also now consumes a token's trailing bucket
   modifier, because stripping only the bracketed half stranded a bare `.monthly` at the
   front of that same real query.

**Deliberately NOT fixed here, filed instead** (both surfaced by the same corpus sweep;
neither is a BL-191 defect and both predate it):

- **BL-197** -- `sanitize_search_query` cannot see a qualified `[table_path::col]` token,
  so the "mandatory" sanitizer no-ops on exactly the real Views that use that form.
- **BL-198** -- View formula removal does not cascade **transitively**: a surviving formula
  is left referencing a deleted one (**48 cases** across 6+ real Views), which
  `tml_lint`'s I13 gate says fails the import with `error_code 14516`.

**Tolerate-old-on-input:** `column_id`, and the `column_id == formulas[].id` binding,
are still read as fallbacks -- the census is one cluster, and **BL-190** tracks the
second-cluster re-run. This follows `thoughtspot-view-tml.md`'s stated policy.
...
plus the census's 42, all read-only, replaying every strip in memory:

| Claim | Measured |
|---|---|
| `view_columns[]` keys ever emitted | exactly `name` / `properties` / `search_output_column` |
| `search_output_column` present | **265 / 265** |
...
~~**Target:** next `tools/ts-cli/` dependency-engine change.~~ Done -- ts-cli v0.127.1,
version bumped in both `__init__.py` and `pyproject.toml` per `.claude/rules/ts-cli.md`.
...
**Second scope addition -- cross-construct-type index collisions (filed with BL-171,
2026-07-30).** `make_resolver` probes three *separate* indexes -- `fact_idx`, `metric_idx`,
`dim_idx` -- in a **fixed priority order** (passthrough fact/metric -> computed fact ->
metric-on-metric -> dimension -> assumed physical), and **nothing anywhere flags a name that
appears in more than one of them.** An SV that declares the same identifier as both a fact and
a dimension (or a metric and a dimension) therefore resolves silently to whichever index is
probed first, and the loser is invisible: the emitted reference is a *valid-looking*
`[TABLE::COL]` or `[formula_<id>]` that points at the wrong construct. That is the same
self-detection class as the rest of this entry -- a wrong reference that no gate can see -- and
it belongs in the same fix: when the column inventory is threaded in, also assert index
disjointness (or emit a ⚑ ambiguity annotation naming both declarations, the way
`_dimension_ref` already does for a renamed passthrough) rather than letting priority order
decide quietly. The priority order itself is correct and documented; what is missing is any
signal that it was *exercised* on an ambiguous name.
...
## BL-197 -- `sanitize_search_query` cannot see a QUALIFIED `[table_path::column]` token, so the "mandatory" sanitizer silently no-ops on real Views `Tier 2`

**Filed:** 2026-07-31.
**Source:** review of the BL-191 fix (PR #427).
**Affects:** `tools/ts-cli/ts_cli/dependency/mutate.py` `sanitize_search_query` (~:66) --
and therefore all three of its callers: `_strip_answer_search_query`,
`_strip_view_search_query`, `_strip_answer_cohorts`.
**Status:** OPEN -- diagnosed, located, and reproduced against real exports. Deliberately
NOT fixed in #427: it is a shared sanitizer on the Answer/View/cohort paths, the fix needs
its own live-import verification, and #427 was scoped to the View *binding*.

**The defect.** The regex is `\s*\[<col>\]\s*` -- the BARE bracket form only. Real
`search_query` values also use the **qualified** form, which the 2026-07-30 census
observed directly:

```
[TS: External Table Info_1::connection_id] ... sum [TS: Daily Row Count_1::row_count] ...
```

Removing the base column `row_count` leaves `[TS: Daily Row Count_1::row_count]` in the
query verbatim. Since ThoughtSpot rejects the import of any Answer/View whose
`search_query` still names a column that no longer exists (open-items #3 -- the reason the
function's own docstring calls it *mandatory*), the sanitizer's contract is not met on
exactly the documents it matters most for.

**Why it survived.** `ts-dependency-manager/SKILL.md`'s troubleshooting table already tells
the operator to *"check for bracket-format variants like `[TABLE::col]` and strip
manually"* -- the gap was known and documented as a manual step, which is how it stayed a
gap. Every unit test uses the bare form.

**The fix.** Match the qualified form as well -- a bracket token whose contents are
`<anything>::<col>` as well as `<col>` -- reusing the whole-token discipline already in the
module rather than a second regex dialect. `migrate/rewrite.py` solved the same problem for
the rename direction with `substitute_bracketed` + `_QUALIFIED_RE`; that is the pattern to
mirror. Note `#427` already extended the regex to consume a trailing bucket modifier
(`[date].monthly`), so the token grammar is in one place.

**Testing.** Unit tests for both forms on all three call sites, plus the combination
(`[TBL_1::col].monthly`). Then a live import of a real View whose `search_query` carries a
qualified token, since this class of change is only trustworthy against the actual parser.
...
## BL-198 -- View formula removal does not cascade TRANSITIVELY, so a surviving formula is left referencing a deleted one `Tier 2`

**Filed:** 2026-07-31.
**Source:** review of the BL-191 fix (PR #427); found by that PR's own corpus sweep over
all 42 real Views on se-thoughtspot.
**Affects:** `tools/ts-cli/ts_cli/dependency/mutate.py` `_strip_view_formulas` (~:340).
Check `remove_columns_from_model_section` at the same time -- it documents that it
*"cascades once"*, which is the same insufficiency one level up.
**Status:** OPEN -- **ready to fix.** Diagnosed, located, and measured. Deliberately not
fixed in #427, which was scoped to the View column/formula *binding*; this is a distinct
single-pass bug that predates it and is unchanged by it.

**The defect.** `_strip_view_formulas` removes formulas whose `expr` references a removed
**column**, in one pass. It does not then remove formulas whose `expr` references a removed
**formula**. ThoughtSpot Views chain formulas freely, so the result is a surviving formula
holding a dangling `[formula_*]` reference -- which `tml_lint.py`'s **I13** gate exists
precisely to catch: *"a bracket id reference matching no `formulas[].id` is parsed as search
tokens, so the formula fails to import (error_code 14516, live-verified 2026-07-30)"*.

**Live measurement** (read-only, zero writes -- the 42-View census corpus, replaying a
removal for every column each formula references):

```
48 transitive dangling cases across 6+ real Views, e.g.
  '2. muze of fis rank...' : remove 'Response Code' -> formula_rank-new still references [formula_count]
  'Growth View'            : remove 'sales'         -> formula_前年比区分 still references [formula_growth]
  'Conditional Triggers2'  : remove 'sales(today)'  -> formula_SalesException still references [formula_Delta]
  'Partner Analysis'       : remove 'date'          -> formula_New labels still references [formula_Moving Sum last 4 quarters]
```

Worked example -- `2. muze of fis rank...`, removing `Response Code`:
`formula_count`, `formula_rank` and `formula_GA-Rank` all reference
`[FIS_Rank Model_1::Response Code]` and are correctly removed; `formula_rank-new`
references `[formula_count]`, is NOT removed, and now dangles.

**The fix.** Iterate to a fixed point: after removing a formula, re-scan for formulas whose
`expr` references any removed formula's **id** (`[formula_X]`) or **name**, and remove those
too, repeating until the set stops growing (bounded by the formula count, so no cycle risk
beyond a visited set). Then re-run the existing `view_columns[]` + `search_query` cleanup
over the full closure -- #427 already routes both through the removed-formula set, so they
come along for free once the set is complete.
...
`search_query` tokens; a cycle fixture to prove termination; and a negative case where an
unrelated formula survives. The corpus sweep in #427's report is the regression oracle --
re-run it and assert 0.
...
The claim is wrong about *Tableau*, not internally inconsistent: the range-join `on:` form
this same file documents twelve lines earlier (`:260-268`) is explicitly the **ThoughtSpot**
side ("ThoughtSpot model TML supports range/inequality joins natively"). So the two halves of
the translation already exist — a source that can express a non-equi join and a target that
can receive one. Only the false premise about the source sits between them.
...
## BL-236 — `lint_tml` crashes on a non-string `joins[].with` instead of reporting it `Tier 4`
