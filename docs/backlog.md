...
| ~~BL-268~~ | ~~`_extract_joins` drops joins with a nested equality expression or a Custom-SQL join side~~ | DONE (2026-08-07) |
...
| BL-269 | `_extract_joins` has no filter for object-graph-cached join duplicates | after BL-268 |
...
## BL-268 -- `_extract_joins` drops joins with a nested equality expression or a Custom-SQL join side `Tier 1` -- **RESOLVED 2026-08-07**
...
cache block (`<object id='_9BBB...'><properties>`) -- see BL-269.

---

## BL-269 -- `_extract_joins` has no filter for object-graph-cached join duplicates `Tier 2`

**Filed:** 2026-08-07. **Jira:** SCAL-330635 (follow-on, filed separately by request).
**Source:** live-verification of BL-268's fix on `Multi level WB v0.twb` -- fixing the
...
After BL-268's fix, `ts tableau parse` on this workbook reports the same join **twice**.
