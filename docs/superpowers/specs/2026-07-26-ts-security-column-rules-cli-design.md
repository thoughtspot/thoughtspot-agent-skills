...
| Code `10023`, body says access is denied instead (`"does not have access"`) | **Live-verified 2026-07-27 (second round, defect 6 above).** Code 10023 is OVERLOADED: this is an access failure, not the feature flag, even though it shares the same code. The caller lacks access to read or modify column security rules in the Org the call ran in; groups and privileges are per-Org, so a token scoped to a tenant Org can lack what the Primary Org token has. **Extended 2026-07-27:** the translation now tells the operator to check for publication BEFORE auditing privileges, because this error is state-dependent -- a tenant read of a published table returns a clean `[]` while no rule exists and `10023` once one does, so it can appear the moment the OWNING Org sets a rule with nothing about the tenant's privileges having changed. That is exactly what stalled the second verification round. The pre-fix build translated every 10023 as the feature flag regardless, which would misdiagnose this case exactly as the 14502 rows below misdiagnosed each other before their own fix. |
| Code `10038`, or the body says `does not have access to read/modify CSR` | **Live-verified 2026-07-27 (`ts-security-columns` round).** A THIRD code, distinct from `10023`. The platform refuses to read or modify CSR for the table in the Org the call ran in. The usual cause is not privileges: the table is PUBLISHED into this Org, and published objects are read-only in target Orgs. Returned even to a caller holding `ADMINISTRATION` there, in the same session that set CSR successfully on that Org's own native table. Note the canonical spec documents this as `403` while the build returns `500`, so the generic 403 fallback never fires for it. Points at `ts share` column grants (CLS), which do apply per-Org. |
