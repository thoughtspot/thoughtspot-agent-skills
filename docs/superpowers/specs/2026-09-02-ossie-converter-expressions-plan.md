...
| **Exemplar templates — 14 of 37 passthrough rows** | `PERCENTILE_CONT`/`DISC` (`0.75`), `APPROX_PERCENTILE` (`0.5`), `NTILE` (`4`), `NTH_VALUE` (`2`), `LAG`/`LEAD` (offset `1`), `TO_TIMESTAMP`/`TO_CHAR` (fixed formats), `DENSE_RANK`/`CUME_DIST` (fixed `ORDER BY`), both typed literals, and window aggregation bake a caller-supplied value in as a literal **while declaring a satisfiable arity**. An argument-count check therefore passes and the wrong constant ships: a model using `PERCENTILE_CONT(0.9)` silently becomes a P75 measure that imports, runs, and is wrong. The design is intended — the catalog holds an exemplar and the caller rebuilds the template per occurrence — and `Construct.template` now documents it. **Plan C/D must rebuild these per occurrence rather than `.format()` them.** |
| **Prose and dispatch templates — ~20 rows** | `CAST`/`TRY_CAST`, the `EXTRACT`/`DATE_TRUNC`/`DATEADD` family, `TRUE, FALSE`, both `CASE` forms, the column/metric reference, the unary row, and several window rows carry prose rather than a substitutable template. All fail **loud** — a `ValueError` from `emit_direct`, or rejection at TML import — so none produces a wrong number. Recorded so a Plan C/D author does not write a uniform `.format()` dispatcher off the type hint. |
| **`spec_construct_names()` ships in the wheel and cannot run from it** | `catalog.py`'s `_find_spec_path()` walks up from `__file__` for `core-spec/expression_language.md`, which is not in the distribution, so it raises `FileNotFoundError` from an installed package. It is a ~230-line test-time oracle inside the runtime package. Moving it to `tests/` is its own change with its own review; deliberately not done in Plan B's fix wave |
| **`_PLACEHOLDER_RE` / `_placeholder_count` duplicated** | Defined identically in `emit.py` and `reverse.py`. Deliberately not extracted: `reverse.py` has no other coupling to `emit.py`, so a shared helper would add an import edge for four lines, and the two are independently-evolving contracts that currently happen to agree |
