...
## Pending upstream — PR #287 would make five of these payload keys first-class

**Status: apache/ossie#287 is OPEN, not merged (last updated 2026-08-27).** Nothing in this
section is spec today. It is recorded because the payload schema above is the part of this
document most exposed to it: [PR #287](https://github.com/apache/ossie/pull/287) ("extended
metadata fields for fields and metrics", `jklahr`, elected PPMC 2026-08-27) proposes eleven
optional non-executional fields, and five of them are things we currently stash under
`vendor_name: THOUGHTSPOT` precisely because the standard had nowhere to put them.

If it lands, those five stop being ThoughtSpot-only losses. **The converter should not emit
them as first-class fields until it does** — a `display_format` on a `Field` fails schema
validation today exactly as a `THOUGHTSPOT` dialect does.

| Stashed today | PR #287 field | Fit |
|---|---|---|
| `data_panel_column_groups` (a map, `{Products: '', Customers: ''}`) | `group_labels` (array of strings) | **Clean.** ThoughtSpot's map keys are folder names and the values are always empty strings, so the map degrades to an array without loss. The array shape is itself downstream of this work — the PR's own wording, "A field may belong to more than one group", was added in response to ThoughtSpot feedback on 2026-08-20 |
| `properties.is_hidden` | `hidden` | **Clean**, with a caveat that is ours, not the spec's: our Model TML reference says not to set `is_hidden` during conversion, because a hidden column cannot be surfaced again without an edit. `Ossie → TML` should therefore keep treating an inbound `hidden: true` as an issue rather than writing it silently |
| `properties.default_date_bucket` | `default_time_granularity` | **Mismatch — `AUTO` has no target.** Ours is `DAILY \| WEEKLY \| MONTHLY \| QUARTERLY \| YEARLY \| AUTO`; the PR's enum is `day \| week \| month \| quarter \| year`. The five named grains map one-for-one, but `AUTO` ("let the engine pick from the query's time span") is a real and common setting with nowhere to go. Under #287 as drafted it stays a declared loss |
| `format_pattern` | `display_format` | **Mismatch — different vocabularies.** The PR specifies an "Excel-compatible display format string" (`$#,##0.00`); ThoughtSpot's `format_pattern` is its own vocabulary. That makes this a *translation* with its own fidelity questions, not a passthrough, and every vendor with a native format vocabulary is in the same position. Whether Excel strings are intended as the canonical interchange vocabulary or merely the illustrative example is unresolved on the PR |
| `currency_type` | `measurement` (`quantity_kind`, `unit` as ISO 4217, `unit_system`) | **Partial.** Our `iso_code` form maps to `measurement.unit` directly. The `column` form — a currency code read from a *sibling column* per row, live-confirmed as `{"column": "TARGET_CURRENCY"}` — has no target: `measurement` describes a static unit, not a per-row one. The `is_browser` form likewise. So the simple case becomes first-class and the two dynamic forms stay stashed |

Two further PR #287 fields touch asks rather than payload keys:

- **`display_label`** answers [**A5**](#open-questions-and-upstream-asks) and narrows [**G2**](ts-ossie-compliance-gaps.md). See A5's row for the re-grading.
- **`default_aggregation`** overlaps the aggregation-default half of the Metric level mapping. It is `fields`-only in the PR, where ThoughtSpot carries `aggregation` on both physical and formula-backed `columns[]` entries, so the overlap is partial.

**What to do with this section.** It is a watch item, not a plan: re-read it when #287 merges, is closed, or changes shape, and fold the resolved rows into the tables above. The two mismatches (`AUTO`, and the format vocabulary) are worth raising on the PR while it is open — they are cheap to fix now and awkward afterwards.
...
- **R4** A metric is always emitted as a formula, never as a physical `column_id` plus an
  ...
  **Evidence class:** a *query-time* semantic, which `VALIDATE_ONLY` cannot probe — both shapes
  ...
  would settle it. Recorded so the correction is not mistaken for a verified finding like **R5**'s.
...
| **A5** | Confirm `label` semantics. **Overtaken by [PR #287](#pending-upstream--pr-287-would-make-five-of-these-payload-keys-first-class) — re-grade before re-asking.** | **That PR adds an explicit `display_label`** ("Human-readable display name for UI and AI interaction") on both `Field` and `Metric`. If it merges the answer is that `label` is *not* the display-name home and `display_label` is — which resolves this ask against our reading, not for it. Two consequences: the mapping's display-name target moves to `display_label`, and **G2** narrows to its identifier half. Do not re-post A5 as written; what remains worth asking is what `label` then means, and that belongs on #287 rather than on the dormant discussion #37 (3 comments, last non-ours 2026-01-21). The original ask, for the record: We read `field.label` as the human display label, matching the Databricks converter's `label` → `display_name` mapping (`converters/databricks/README.md:89`). `core-spec/spec.md:236` describes it as "Label for categorization", which reads differently. ThoughtSpot needs a display-label home because its display names are not valid SQL identifiers. | **#287** (was: discussion #37 (Display name); related #31) |
...
lives in `expr`, with the column-level `aggregation` present only as a convention. That
`SUM` is inert **here specifically because this `expr` is already an aggregate**
(`sum ( [ORDERS::Amount] )`) — *per ThoughtSpot domain review, 2026-07-30*, a column-level
`aggregation` is a no-op only over an aggregate `expr`; over a **scalar** one it applies, and the
formula is rolled up by it. Had this worked shape emitted pattern B of [**R4-P3**](#reverse-direction-rules-ossie--tml)
— a scalar `expr` plus `aggregation: SUM` — that `SUM` would be load-bearing, not conventional.
(A query-time semantic, so no import probe can test it either way.)
