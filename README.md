...
| [`ts-profile-domo`](agents/cli/ts-profile-domo/SKILL.md) | Add, update, test, or delete Domo profiles — developer-token auth | Credentials | [✓](agents/cli/ts-profile-domo/SKILL.md) | — | — |
...
| [`ts-convert-from-domo`](agents/cli/ts-convert-from-domo/SKILL.md) | Convert a captured offline Domo bundle (dataset schemas, Beast Modes, cards, page) into ThoughtSpot Table + Model TMLs and Answers plus a single-page Liveboard, translating Beast Mode formulas (SQL pass-throughs where ThoughtSpot has no equivalent) and taking model joins from a Magic ETL export | Domo | [coverage](agents/cli/ts-convert-from-domo/references/coverage-matrix.md) | [✓](agents/cli/ts-convert-from-domo/SKILL.md) | — | — |
