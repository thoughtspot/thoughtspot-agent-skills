...
|---|---|---|
| 1.4.3 | 2026-09-22 | **I7 untranslatable gate added.** The Step 10 Unmapped Properties Report told the user which columns were omitted with no instruction to check [ts-databricks-formula-translation.md](../../shared/mappings/ts-databricks/ts-databricks-formula-translation.md) before relaying the CLI's verdict. The gate names the `TS → Databricks` tables and excludes the reverse section by name — the wrong side for this direction. Now gated by `check_i7_gate.py`, which requires the literal `MANDATORY (I7)` marker in a blockquote citing this skill's own dialect mapping and the invariants doc (2026-09-22 audit finding 9.3). |
