...
| **18.1+** | Join `cardinality:` and window `offset:` | Required only if the model has an explicit `ONE_TO_MANY` join (emits `cardinality: one_to_many`) or a period-over-period measure — prior month/quarter/year (emits window `offset:`). A `MANY_TO_ONE` join does **not** trigger this tier: as of v1.4.0 it emits the runtime-agnostic `rely: { at_most_one_match: true }` and no `cardinality:` key (BL-174) |
...
month/quarter/year) or an explicit ONE_TO_MANY join, 18.1+ is needed instead.
