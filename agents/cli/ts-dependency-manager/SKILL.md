...
| View TML import fails after column removal | Check for a **dangling `view_columns[]` entry** — one whose `search_output_column` still names a removed column or a dropped `formulas[].name`. `view_columns[]` binds on `search_output_column` (a search-output label, which may carry a `Total X` / `Month(X)` decoration), **not** on `column_id` — that field does not exist in real View TML (BL-191). Confirm against [../../shared/schemas/thoughtspot-view-tml.md](../../shared/schemas/thoughtspot-view-tml.md) |
