...
| 1.10.3 | 2026-07-29 | **TPC-DS conversion-fidelity cross-validation — seven coverage-matrix corrections (docs only; the behaviour fixes are BL-174/BL-176/BL-180).** Round-tripping apache/ossie's community-reviewed TPC-DS Metric View fixture through this converter and back (`docs/reviews/2026-07-29-ossie-tpcds-fidelity.md` §2) found five defects the matrix did not declare. New row **#77** — the join **type** is not an MV field and Databricks fixes star-schema joins as `LEFT OUTER`, but `mv_build_model.py:236` emits `INNER` unconditionally, so measures read **lower** in ThoughtSpot than in Databricks whenever a fact FK is NULL or unmatched (BL-174). New row **#78** — a dimension/measure `name:` is discarded whenever `display_name` is present, so the MV's queryable identifier is unrecoverable on the reverse leg (BL-166). New row **#79** — measure `format:` is parsed and translated and then never written to `properties.currency_type`, although `ts-databricks-properties.md:109`/`:122` document the pair as mapped and the to-direction already emits it (BL-174). Row **#13** corrected — `cardinality`/`rely` *are* written to TML (`MANY_TO_ONE` on every join), which silently raises the round-tripped MV's Databricks Runtime floor from 17.3+ to 18.1+, not "informational — not written to TML" (BL-174). Rows **#23/#67** now state the semantics change they cause: `x / NULLIF(y,0)` returns NULL on a zero divisor and `safe_divide` returns 0 (BL-180). New limitation **L11** — the file-only path defaults every numeric column to `MEASURE`/`SUM`, so surrogate keys and MV-declared dimensions arrive as summable measures (BL-176). No behaviour change in this release. |
| 1.10.2 | 2026-07-29 | **BL-170 — six string functions corrected to pass-throughs/compositions (docs only; CLI fix is BL-171).** Live verification on se-thoughtspot 2026-07-29 proved `trim`, `ltrim`, `rtrim`, `replace`, `starts_with` and `ends_with` are **all** absent from the ThoughtSpot formula parser. `ts-databricks-formula-translation.md` now maps `TRIM`/`LTRIM`/`RTRIM`/`REPLACE` to `sql_string_op` pass-throughs and `STARTSWITH`/`ENDSWITH` to the `strpos ( ) = 1` / `substr`-`strlen` compositions; `references/coverage-matrix.md` #40/#41 were split, with the corrected mappings in new rows #75/#76. **Caveat: `ts_cli/databricks/mv_sql.py`'s `_RENAME` still emits the bare `trim`/`ltrim`/`rtrim`/`replace`/`starts_with`, so those formulas fail at import until BL-171 lands (`ENDSWITH` has no `_RENAME` entry and is simply unmapped).** |
