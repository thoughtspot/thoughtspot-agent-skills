...
| 79 | `format: {type: currency, currency_code: USD}` on a **measure** | `properties.currency_type.iso_code` | Written as of ts-cli v0.127.0 (BL-174). `type: percentage`, `decimal_places`, and a `format:` on a `fields:`/`dimensions:` entry are **not** mapped — `currency_type` is a measure property (`thoughtspot-model-tml.md`, *`columns[]` fields*) and percentage formatting has no ThoughtSpot equivalent; those cases are tolerated and dropped (see limitation L12). |
...
| 13 | `joins[].cardinality` / `joins[].rely` | `joins[].cardinality` (`MANY_TO_ONE` / `ONE_TO_MANY`) | Parsed, precedence applied, **written to TML on every join** — including one whose source declared only the runtime-agnostic `rely: {at_most_one_match: true}` hint, or neither. ThoughtSpot **requires** the field on an inline join: omitting it fails with "both  type and cardinality should be defined" (live-probed 2026-07-31, `import_policy: VALIDATE_ONLY`), so BL-174's original "omit it" remedy is not available. The round-trip cost it was filed for is closed on the **to**-direction instead: as of ts-cli v0.127.0 the reverse leg maps `MANY_TO_ONE` back to `rely:` and writes **no** MV `cardinality:` key, so a round trip keeps the MV's Databricks Runtime floor at 17.3+ rather than raising it to 18.1+. |
| 77 | Join **type** — not an MV field; Databricks fixes star-schema joins as `LEFT OUTER` | `joins[].type: LEFT_OUTER` | Emitted on every join (including nested ones) as of ts-cli v0.127.0 (BL-174). Was `INNER`, which **dropped** every fact row whose FK is NULL or matches no dimension row — the Metric View keeps those rows, so measures read *lower* in ThoughtSpot than in Databricks on the same data. Source: *"In a star schema, the `source` is the fact table and joins with one or more dimension tables using a `LEFT OUTER JOIN`"* ([Joins in metric views](https://docs.databricks.com/aws/en/business-semantics/metric-views/joins), re-confirmed 2026-07-31); `LEFT_OUTER` accepted live on a Model inline join the same day. |
...
| L12 | `format:` other than a **currency on a measure** — i.e. `type: percentage`, `decimal_places`, or any `format:` on a `fields:`/`dimensions:` entry | Tolerated and dropped, **silently** — nothing reports it in `build-model`'s diagnostics (BL-196). `properties.currency_type` is a MEASURE property taking one of `iso_code`/`column`/`is_browser` (`thoughtspot-model-tml.md`, *`columns[]` fields*) — it has no place for a decimal-place count and no percentage form, and an ATTRIBUTE's currency formatting would need `format_pattern`, which is not auto-emitted (`ts-databricks-properties.md`, *MV fields with no TS equivalent* → the `format:` row) | Set the display format on the ThoughtSpot column after import. The currency measure case (row #79) is converted automatically |
