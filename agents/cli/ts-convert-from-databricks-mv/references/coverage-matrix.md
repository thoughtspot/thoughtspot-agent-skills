...
| L12 | `format:` other than a **currency on a measure** — i.e. `type: percentage`, `decimal_places`, or any `format:` on a `fields:`/`dimensions:` entry | Tolerated and dropped. `properties.currency_type` is a MEASURE property taking one of `iso_code`/`column`/`is_browser` (`thoughtspot-model-tml.md:232`) — it has no place for a decimal-place count and no percentage form, and an ATTRIBUTE's currency formatting would need `format_pattern`, which is not auto-emitted (`ts-databricks-properties.md:122`) | Set the display format on the ThoughtSpot column after import. The currency measure case (row #79) is converted automatically |
