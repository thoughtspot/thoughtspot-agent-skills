...
|---|---|---|
| 1.3.1 | 2026-07-08 | Correct CTE set-operation limitation (engineering-confirmed, live-verified on nebula-spotQL): set operations inside a user-defined CTE work when no branch contains an aggregate measure; only aggregated branches fail. Note `10000: Failed to transform QuerySpec: null` on outer re-aggregation (likely SCAL-318834). |
