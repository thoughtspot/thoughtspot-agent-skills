...
| 2 | Custom SQL relations (`<relation type="text">`) | SQL View TML | Full SQL preserved in `sql_query`. A relation name is unique only within a datasource, so several datasources can each emit `Custom SQL Query`; a name contested by more than one datasource — or by a physical table — is qualified `Base (Datasource)` (`Base (Datasource N)` if that is taken), on **every** owner, so it does not depend on datasource order. Uncontested names are unchanged; physical tables are never renamed; `model_tables[]`, `column_id`, joins and formula refs follow. A table and a SQL View sharing one name *within one datasource* is not fully separable (BL-284) — Tableau does not emit that shape. GENERATE mode only: `--existing-guid` defers to the names the target model already uses and reconciles neither direction (BL-313). SCAL-339750, ts-cli v0.147.0 |
...
| 137 | Tableau internal pseudo-field `:Measure Names` | Excluded from Liveboard/Answer fields at parse time; `build-model` (GENERATE) additionally excludes them from Model and Table TML and drops formulas referencing them | It names no warehouse column, so it emitted a `column_id` resolving to nothing, and no gate saw it in either shape: on a single-table model the id comes out table-qualified and the same phantom is written into that table's TML, so the cross-reference check resolves it; on a genuine multi-table model no ownership resolves, so the id is bare and reaches no Table TML — and I12, which flags a bare `column_id`, is scoped to single-table models. Matched by **exact name**, never by leading-colon prefix, so a user column such as `:Custom Thing` is kept. `Multiple Values` — the other half of Tableau's Measure Values pivot — is **not** excluded as a column: it is recognised as a shelf token (so a pivot worksheet still resolves to its real measures, and neither token is emitted as a Liveboard field), but it has never been observed as a `<column>` and is a name a warehouse column could legitimately carry. Tableau's built-in `Number of Records` (see #48) is a calculated field and is skipped before this predicate runs; a warehouse column genuinely named that is not a calculated field and survives. A hand-assembled blend-merged model must apply the same exclusion itself: the parse's `columns[]` carries these, and only `build-model` filters them. SCAL-338494, ts-cli v0.148.0 |
...
| 135 | `TRIM(s)` | `sql_string_op ( "TRIM({0})" , s )` | Bare `trim(...)` is NOT a valid ThoughtSpot formula function — live-verified 2026-07-29, se-thoughtspot (BL-170). Scalar pass-through (PT1), CLI-translated (v0.126.1 — BL-171; the emitted form was live-verified on se-thoughtspot 2026-07-30). Previously (incorrectly) documented as native `trim(...)` (former #18) |
| 136 | `LTRIM(s)` / `RTRIM(s)` | `sql_string_op ( "LTRIM({0})" / "RTRIM({0})" , s )` | No native `ltrim`/`rtrim` — live-verified 2026-07-29, se-thoughtspot (BL-170). Scalar pass-through (PT1), CLI-translated (v0.126.1 — BL-171; both emitted forms live-verified on se-thoughtspot 2026-07-30) |
...
| U7 | `FULLNAME()` / `ISFULLNAME(s)` / `USERDOMAIN()` | Rejected with reason at translate time (ts-cli v0.26.0) — manual translation required. `USERNAME()`/`ISUSERNAME(s)` (formerly also U7) are no longer in this list as of v0.88.0 — see #134 in Mapped Constructs. `FULLNAME`/`ISFULLNAME` stay rejected (no confirmed ThoughtSpot display-name variable); `USERDOMAIN` stays rejected (`ts_email_domain` is a candidate but the domain-only-vs-full-email value shape is unverified). Deferred, BL-071 |
| U9 | `USERATTRIBUTE(attr)` / `USERATTRIBUTEINCLUDES(attr, val)` | Rejected with reason at translate time (ts-cli v0.28.1) — sibling of U7; ABAC `ts_var()` referencing a formula variable is a plausible native translation, but `ts_var(...)` is only accepted in RLS RULES, not in Model/Answer formulas today, so no faithful in-formula translation exists yet. Deferred, BL-071 |
