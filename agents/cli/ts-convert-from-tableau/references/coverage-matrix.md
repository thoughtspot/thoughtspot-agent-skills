...
| 22 | Tableau internal pseudo-fields `:Measure Names` / `Multiple Values` | Excluded during column cleanup (not emitted to Model or Table TML), from Liveboard/Answer fields, and from formulas that reference them | It names no warehouse column, so it emitted a `column_id` resolving to nothing — every I12 `ts tml lint` finding, plus the same column table-qualified on multi-table models where I12 is scoped out. Matched by **exact name**, not by leading-colon prefix, so a user column such as `:Custom Thing` is kept; `:Measure Names` is the only pseudo-field established, not all of them. `Number of Records` is unaffected — it is a real measure, see #48. SCAL-338494, ts-cli v0.148.0 |
