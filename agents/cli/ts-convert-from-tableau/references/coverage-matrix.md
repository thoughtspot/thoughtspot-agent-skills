...
| 22 | Tableau internal pseudo-fields `:Measure Names` / `Multiple Values` | Excluded from Liveboard/Answer fields at parse time; `build-model` (GENERATE) additionally excludes them from Model and Table TML and drops formulas referencing them | Neither names a warehouse column, so each emitted a `column_id` resolving to nothing — every I12 `ts tml lint` finding, plus the same column table-qualified on multi-table models where I12 is scoped out. Both are matched by **exact name**, never by leading-colon prefix, so a user column such as `:Custom Thing` is kept. **Limitation:** `Multiple Values` carries no marker of its own, so a warehouse column literally named that is excluded too — exact matching is the deliberate trade against the broader false positives a prefix or substring rule would cause. Tableau's built-in `Number of Records` is a calculated field and is skipped before this predicate runs; a warehouse column genuinely named that is not a calculated field and survives (see #48). A hand-assembled blend-merged model must apply the same exclusion itself: the parse's `columns[]` carries these, and only `build-model` filters them. SCAL-338494, ts-cli v0.148.0 |
