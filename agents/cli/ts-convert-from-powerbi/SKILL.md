...
|---|---|---|
| 1.2.0 | 2026-09-24 | **Gated measure shapes convert instead of being flagged.** A gated distinct count (`CALCULATE(DISTINCTCOUNT(T[key]), NOT ISBLANK(T[gate]))`) and a gated ratio over one grain (the `VAR`/`VAR`/`RETURN a/b` form, including an `IN {…}` value set) are mechanical and fully described by the DAX, so the review gate consulting `ts_cli/powerbi/gated.py` before giving up turns them from hand-rebuild into Migrated. Wrapped in `USERELATIONSHIP` they are **Approximated**: right for the grain, wrong for a page slicing on the role-playing date, which needs a union fact keyed by measure. On a real 27-measure report this took measures translated from 2 to 19 of 27 and the generated liveboard from 33 to 65 tiles. ts-cli v0.147.0. |
