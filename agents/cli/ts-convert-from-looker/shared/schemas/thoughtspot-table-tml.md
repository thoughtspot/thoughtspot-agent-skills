...
**The source model's declared type is a starting guess, not ground truth — the CDW error
message is authoritative.** When converting from a semantic layer (LookML, Snowflake SV,
Tableau, …), the source's own type annotation can be wrong relative to the live warehouse
column: a LookML `type: string` field can back a genuinely numeric Snowflake `NUMBER`
column, and a LookML `type: number` field can back a `VARCHAR` column (e.g. a
sparse/nullable identifier stored as text). `DataType {X} does not match CDW DataType`
means *X is wrong for this specific column*, not that the whole approach is broken — the
fix is almost always a one-line `data_type:` swap on that single column, not a rethink of
the join or model. If your first guess (based on the source type) fails, try the source
type declaration's most likely CDW rendering (`INT64` ↔ `VARCHAR` ↔ `DOUBLE`) rather than
assuming the column can't be represented. Retry with `--policy VALIDATE_ONLY` on just that
one Table TML to iterate quickly without touching already-passing tables. Two real
examples from one LookML project (both against the same Snowflake connection):
`products.distribution_center_id` declared `type: string` in LookML → actually `NUMBER`
in Snowflake, fixed by switching `VARCHAR` → `INT64`; `events.ad_event_id` declared
`type: number` in LookML → actually text in Snowflake, fixed by switching `INT64` (then
`DOUBLE`) → `VARCHAR`.
