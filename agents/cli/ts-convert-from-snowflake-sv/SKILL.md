...
|---|---|---|
| 1.22.3 | 2026-09-22 | **I7 untranslatable gate added.** The skill reached its Step 9 `skipped[]` surfacing with no instruction to open [ts-snowflake-formula-translation.md](../../shared/mappings/ts-snowflake/ts-snowflake-formula-translation.md) first, so an expression with a documented ThoughtSpot equivalent could be dropped on syntax recognition alone. Now gated by `check_i7_gate.py` (2026-09-22 audit finding 9.3 — the invariant was enforced by nothing and missing from 9 of 11 converters). |
