...
|---|---|---|
| 1.22.3 | 2026-09-22 | **I7 untranslatable gate added.** Step 9 surfaced `skipped[]` and asked whether to proceed without them, with no instruction to open [ts-snowflake-formula-translation.md](../../shared/mappings/ts-snowflake/ts-snowflake-formula-translation.md) first — while this skill's own CoCo mirror had carried that gate since v1.3.1, and the invariants doc names this skill as an establishing source for I7. Now gated by `check_i7_gate.py`, which requires the literal `MANDATORY (I7)` marker in a blockquote citing this skill's own dialect mapping and the invariants doc (2026-09-22 audit finding 9.3). |
| 1.22.2 | 2026-09-15 | **Two silent parser drops, each losing a whole field of every real Semantic View (ts-cli v0.138.0).** **BL-254:** `ai_sql_generation` / `ai_question_categorization` were matched by a pattern requiring `=`. Snowflake **rejects** that spelling outright (`syntax error ... unexpected '='`) and `GET_DDL` emits the bare form, so the patterns were unmatchable **by construction** — `custom_instructions` came back `None` on every input for 82 ts-cli releases (v0.63.0 through v0.137.0), and Step 4's instruction to "log as GAP-06 if `ai_sql_generation` is present" could never fire. The unit fixture asserted the `=` form, so the suite **confirmed** the bug rather than catching it. **BL-268 (new):** `ai_verified_queries` sat in `_extract_top_level_comment`'s block list, i.e. among the clauses the top-level `comment=` is assumed to follow — but Snowflake requires the comment **before** it and rejects the reverse, so on any view carrying verified queries the search window opened past the comment and the Model imported with only the converter's `"Converted from ..."` boilerplate as its description. Distinct from BL-255/N2, which is the *raw-scan* mechanism (comment text containing a clause name) and remains open: the new fixture's comment is `'Order analytics across channels'` — no clause name, no paren — and still lost it. Both defects exit 0 with `unsupported: []`, `warnings: []`, `lint_findings: []`. Found converting a live customer SV that silently lost its 1,923-character instruction block and its entire description. Coverage-matrix rows 6 and L1 and open-items #2 corrected — #2 had read **Done** since v1.16.0 for a parse that had never run. |
| 1.22.1 | 2026-09-02 | **A relationship's join keys are named in the SV namespace, not the physical column — two of eight joins referenced a column that does not exist (ts-cli v0.137.0).** `_build_join_on` passed `relationships[].from_cols`/`to_cols` through verbatim, but a `relationships(...)` clause names its keys as SV *constructs*, and those differ from the physical column whenever the construct renames one: `DM_ORDER_DETAIL.DM_ORDER_DETAIL_ORDER_ID as dm_order_detail.RRDER_ID` referenced by the relationship as `DM_ORDER_DETAIL(DM_ORDER_DETAIL_ORDER_ID)` emitted `[DM_ORDER_DETAIL::RRDER_ID]`'s logical name against a table whose column is `RRDER_ID`. Caught converting `TEST_SV_DUNDER_MIFFLIN_SALES_INVENTORY` live: the two affected joins were order-line→order-header and order→date, i.e. revenue could not be grouped by order or by date at all — the model's primary axes. **Every gate was green**: `build-model` reported `lint_findings: []`, and the XREF cross-reference check only runs when Table TMLs are linted *alongside* the model, which never happens when tables came from `ts tables create` rather than TML on disk. The `columns[]` resolver already handled renames (0 broken `column_id` vs 2 broken join refs), so only the join builder was wrong. New `build_join_column_index` mirrors `sv_translate._dimension_ref`'s three-way split — passthrough and bare-column rename resolve; a computed construct is deliberately NOT indexed, since it cannot be a join key and passing it through unresolved makes an SV that tries one fail loudly. Databricks is unaffected: an MV's `on` clause names physical columns. **Second, related defect found in the same conversion:** a rename written with a qualified and/or quoted identifier — `DM_DATE_DIM.DATE_VALUE as dm_date_dim."DATE"` — was classified as *computed* and emitted as a formula, because the bare-column detector only matched an unqualified, unquoted identifier. `_translate_dimension`'s own docstring names the consequence ("a table needs at least one real column selected or it imports with a cross-join warning"): on a date dimension whose only other references are join keys, it left `DM_DATE_DIM` with **zero** `column_id` entries and ThoughtSpot refused the import outright — *"One or more tables have been added to the model without selecting any of their columns ... DM_DATE_DIM"*. BL-212 taught the expression *translator* to read quoted identifiers; this is the *classifier* catching up. New `bare_column_name` lives in `formula_common.py` and is shared by both classifier sites and the join index, so the three cannot disagree about what "names one column" means. |
