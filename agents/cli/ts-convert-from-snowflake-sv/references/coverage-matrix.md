...
| 27 | Double aggregation / metric-on-metric (`AVG(table.count_metric)`) | `average(group_count([T::col], [DIM::pk]))` | `group_*` shorthands, grouping on the PK of the parent (TO) side of the relationship connecting the two metrics' tables; full `group_aggregate(..., {key}, query_filters())` form when the inner aggregation has no shorthand or the inner expression is not a simple aggregate. When the two tables are **not** related there is no grouping key, and the reference falls back to the inner metric's `[formula_<id>]`. Fixed and live-re-verified 2026-07-30 (BL-178). |
