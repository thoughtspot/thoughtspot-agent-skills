...
| `metrics ( NAME as m1 / m2 )` — **unqualified derived metric**, no table prefix on the left | `formulas[]` MEASURE. The only SV construct that can span two **unrelated** facts (a qualified metric may reference only metrics on directly related entities — Snowflake `010211`), so it is how attainment / growth ratios are expressed. Each reference resolves to what that metric is *emitted* as: a simple `AGG(col)` metric to its column display name `[Amount]`, anything else to `[formula_<id>]`. Note `DIV0()` is rejected inside a derived metric (`010271`) — the SV must use plain `/`. |
