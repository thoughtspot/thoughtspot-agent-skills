---
name: ts-promote-answer-to-model
...
| [~/.claude/skills/ts-profile-setup/SKILL.md](~/.claude/skills/ts-profile-setup/SKILL.md) | ThoughtSpot auth, profile config, token persistence |
...
user to run `/ts-profile-setup` first.
...
[ts-profile-setup/SKILL.md](~/.claude/skills/ts-profile-setup/SKILL.md) for the token
refresh procedure.
...
Consider migrating the Worksheet to a Model first using /ts-model-builder,
...
| Symptom | Action |
|---|---|
| `ts auth whoami` returns 401 | Token expired — follow the refresh steps in `/ts-profile-setup` |
| Answer TML has no `formulas[]` | Answer may not have custom formulas — see Step 3 |
| Data source is a Worksheet | Worksheets are legacy — inform user, suggest `/ts-model-builder` to migrate first |
