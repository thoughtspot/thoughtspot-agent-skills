...
> you are in a Snowsight Workspace, inform the user they must run `/setup-ts-profile`
...
(see [~/.claude/skills/setup-ts-profile/SKILL.md](~/.claude/skills/setup-ts-profile/SKILL.md) — Pattern A):
...
- **Temp file (preferred for multi-call scripts):** Authenticate using Pattern A in
  [~/.claude/skills/setup-ts-profile/SKILL.md](~/.claude/skills/setup-ts-profile/SKILL.md) — writes the token
