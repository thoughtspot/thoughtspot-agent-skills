...
> you are in a Snowsight Workspace, inform the user they must run `/ts-setup-profile`
...
(see [~/.claude/skills/ts-setup-profile/SKILL.md](~/.claude/skills/ts-setup-profile/SKILL.md) — Pattern A):
...
- **Temp file (preferred for multi-call scripts):** Authenticate using Pattern A in
  [~/.claude/skills/ts-setup-profile/SKILL.md](~/.claude/skills/ts-setup-profile/SKILL.md) — writes the token
