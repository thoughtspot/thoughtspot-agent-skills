...
```bash
ts connections list --profile {ts_profile_name} --type SNOWFLAKE
```
...
```bash
ts connections get {ts_connection_id} --profile {ts_profile_name}
```
...
Write the new/updated tables as a JSON array to `/tmp/ts_new_tables.json` using this format:
...
Then run — the CLI handles fetch, merge, and update internally:

```bash
ts connections add-tables {ts_connection_id} --profile {ts_profile_name} < /tmp/ts_new_tables.json
rm /tmp/ts_new_tables.json
```
...
```bash
ts metadata search --profile {ts_profile_name} --type LOGICAL_TABLE
```
Filter the JSON output by name matching the table name.
...
Write each table TML to a temp file, then import all at once:

```bash
python3 -c "import json,sys; print(json.dumps([open(f).read() for f in sys.argv[1:]]))" \
  /tmp/ts_table_*.yaml | ts tml import --profile {ts_profile_name} --policy PARTIAL --create-new
rm /tmp/ts_table_*.yaml
```

Parse the JSON response — each object has its own status. Report any failures before
...
```bash
python3 -c "import json,sys; print(json.dumps([open(f).read() for f in sys.argv[1:]]))" \
  /tmp/ts_model.yaml | ts tml import --profile {ts_profile_name} --policy ALL_OR_NONE --create-new
rm /tmp/ts_model.yaml
```
