...
Profiles are used by `ts-to-unity-catalog` and other skills that connect to Databricks.
...
if not profiles:
    print("No Databricks profiles configured. Run /databricks-profile-setup first.")
    exit()
elif len(profiles) == 1:
    profile = profiles[0]
...
Your Databricks token may have expired. To refresh:
  ...
  3. Run /databricks-profile-setup → T (Test) or U → Refresh token
```
