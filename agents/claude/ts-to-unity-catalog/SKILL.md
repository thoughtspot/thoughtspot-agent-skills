...
| [~/.claude/skills/databricks-profile-setup/SKILL.md](~/.claude/skills/databricks-profile-setup/SKILL.md) | Databricks connection code, SQL execution patterns, token refresh |
...
Can you log into ThoughtSpot in a browser (even via SSO)?
  ...
  NO  → password_env or secret_key_env — see ts-profile-setup
```
...
- Connection configured — run `/databricks-profile-setup` if you haven't already
...
[~/.claude/skills/databricks-profile-setup/SKILL.md](~/.claude/skills/databricks-profile-setup/SKILL.md).
...
| `[DATABRICKS_TOKEN_*] is empty` | Token expired | See token refresh instructions in databricks-profile-setup |
