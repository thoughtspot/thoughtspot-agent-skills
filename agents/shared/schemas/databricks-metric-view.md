...
| **17.3+** | Agent metadata (`display_name` / `comment` / `synonyms` — which this repo's converters emit); `materialization:` block (GA — corrected 2026-07-29, finding 13.7; was documented as Public Preview); **Snowflake-schema joins**, **`TEMPORARY` metric views**, **JDBC/ODBC (Thrift) support** (added 2026-08-26, finding 13.20) |
| **18.0+** | `REFRESH MATERIALIZED VIEW` support for a Metric View's materializations (finding 13.7); **BI compatibility mode** (added 2026-08-26, finding 13.20) |
...
> **Why the Snowflake-schema row matters** (finding 13.20). This repo makes nested multi-hop joins the documented mechanism and the converter emits them, while the table above gives 16.4 as the floor for MVs to run at all. Nobody is broken today because the agent metadata this repo always emits already forces 17.3+ — but that is a coincidence, not a guarantee. It becomes load-bearing the moment a bare-metadata emission mode is added, so the floor is recorded here rather than left implied.
...
**Join type is fixed, not declared (confirmed 2026-07-31).** There is no `type:` field on
a join node, because Databricks fixes the semantics: *"In a star schema, the `source` is
the fact table and joins with one or more dimension tables using a `LEFT OUTER JOIN`"*
([Joins in metric views](https://docs.databricks.com/aws/en/business-semantics/metric-views/joins)).
Fact rows whose FK is NULL or matches no dimension row are therefore **retained**. A
converter emitting these joins into a format that *does* carry a join type must emit the
left-outer equivalent — anything narrower silently drops rows and lowers every measure
(this was BL-174 defect 1: `INNER` in ThoughtSpot Model TML).
...
### v1.1 strips `#` comments on save (finding 13.21, 2026-08-26)

Databricks' v1.1 spec states that any single-line `#` comments in a metric-view
definition **are removed when the definition is saved**. Two consequences:

* **Comments cannot carry provenance.** A `# generated by ts-convert-to-databricks-mv`
  line would not survive, so anything that needs to persist belongs in the `comment:`
  field (a real YAML key, retained) — never in a `#` comment.
* **A re-read of a created MV will not byte-match what was submitted.** Any future
  round-trip or idempotence check must compare *parsed structure*, never raw text.

**This repo's emitters are unaffected**, and the reason is worth recording rather than
re-derived: `mv_build_view.py` serialises via `yaml.safe_dump` on a dict, which cannot
emit comments at all. So no generated DDL has ever carried one. The constraint applies to
the hand-annotated YAML *in this file* and to anyone hand-editing a definition — not to
our output.
