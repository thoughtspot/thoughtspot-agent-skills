...
| CL08 | `Mixmatch(expr, val1, val2)` | `sql_bool_op ( "LOWER({0}) IN (LOWER({1}), LOWER({2}))" , col , val1 , val2 )` | corrected | Qlik Mixmatch is case-insensitive Match. The previous entry was wrong twice over: **`lower()` is not a native ThoughtSpot function** (verified 2026-06-13) and the membership list used round parentheses, which the formula parser rejects (live-verified 2026-07-29, se-thoughtspot — BL-170). Case-insensitive matching has no native form at all, so the whole test goes inside one `sql_bool_op` — the round parens above are **warehouse SQL**, not ThoughtSpot `in { }` syntax. Same treatment as `ILIKE`. |
...
| D09 | `Minute(time)` | `sql_int_op ( "MINUTE({0})" , col )` | corrected | ThoughtSpot has **no minute extractor** (live-verified 2026-07-30 — BL-171: `minute (` is rejected). Push it to the warehouse with a `sql_int_op` pass-through, the same treatment `ts_cli/sv_sql.py` and `mv_sql.py` give SQL `MINUTE()`. | **Quoting corrected 2026-08-26 (finding 17.2):** those two modules emit a **double**-quoted outer template (`sv_sql.py:388`, `mv_sql.py:438`), so this row's previous single-quoted form contradicted the precedent it cites.
...
| S12 | `Index(str, substr, n)` | `strpos(col, substr)` | corrected | ThoughtSpot `strpos` returns the position of the **first** occurrence (1-indexed, 0 when not found), which is exactly Qlik `Index()` with its default `n=1`, so the CLI maps the 2-argument form. A **third argument** (nth occurrence, including a negative n counting from the end) has no ThoughtSpot equivalent and is **flagged NEEDS REVIEW at translate time** — a bare rename instead passed it through as `strpos(col, sub, 2)`, a real function with the wrong arity, so the conversion reported success and the *import* failed later — live-confirmed 2026-07-30 on se-thoughtspot: `Function strpos expects only 2 arguments.` (error_code 14516), BL-171. The earlier "no substring-position function" claim was also wrong. |
