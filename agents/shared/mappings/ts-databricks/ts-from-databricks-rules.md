...
adds a redundant `cardinality:` key (BL-174). Note the floor claim once attached to this
round trip was withdrawn 2026-08-26 (finding 13.13): `rely:` itself is listed under 18.1 in
the vendor availability matrix, so avoiding `cardinality:` does not demonstrably keep the MV
at 17.3+ — see [databricks-metric-view.md](../../schemas/databricks-metric-view.md) "The
`rely` Runtime Floor".
