...
| `properties.currency_type` | `format: { type: currency, currency_code: <iso> }` | v1.1 supports `format:` on measures, and (corrected 2026-07-29, finding 13.10) also on `fields:`/`dimensions:` entries — not measures-only. The ThoughtSpot side of the pair is `currency_type.iso_code` (**not** `currency_code`); implemented in both directions as of 2026-07-31 (BL-174) |
...
| `rely: { at_most_one_match }` | **Mapped** | → `cardinality: MANY_TO_ONE` on the TS join (which the platform *requires* — "both  type and cardinality should be defined", live-probed 2026-07-31). The reverse leg maps it back to `rely:` and writes no MV `cardinality:` key, so a round trip carries no redundant `cardinality:` key (BL-174). The "keeps the 17.3+
Runtime floor" claim previously made here was withdrawn 2026-08-26 (finding 13.13) — `rely:`
is itself listed under 18.1 in the vendor availability matrix; see
[../../schemas/databricks-metric-view.md](../../schemas/databricks-metric-view.md) "The
`rely` Runtime Floor" |
| `format:` (currency/percentage) — measures OR fields/dimensions (corrected 2026-07-29, finding 13.10) | **Partial** | `format: {type: currency, currency_code: X}` on a **measure** → `properties.currency_type.iso_code` (implemented 2026-07-31, BL-174, both directions). A formatted *dimension*'s `format:` is tolerated and dropped — `currency_type` is a measure property; mapping it to the ATTRIBUTE `format_pattern` is not yet auto-emitted. `decimal_places` and `type: percentage` have no TS equivalent. **The drop is silent** — this row said "tolerated and logged" from 2026-07-29, but no code path logs it and `build-model` has no warnings channel for it; filed as **BL-196** |
...
| `materialization:` (top-level block: `schedule`, `mode`, `materialized_views[]` incl. `cluster_by`/`partition_by`) | **Metadata only** | Databricks-side query-acceleration hint — **GA at Runtime 17.3+ (corrected 2026-07-29, finding 13.7; was Public Preview)** — no ThoughtSpot analog; not stored on import. See [databricks-metric-view.md](../../schemas/databricks-metric-view.md#materialization-block-ga--runtime-173). Docs-research finding (Task 1, `docs/audit/2026-07-08-dbx-window-docs-findings.md`), 2026-07-08; GA/cluster_by/partition_by correction docs-research only, 2026-07-29 |
...
| Param Filter | `[Date Parameter]` | MV `parameters:` is a GA construct (Runtime 18.2+, mutually exclusive with the `materialization:` block — see [databricks-metric-view.md](../../schemas/databricks-metric-view.md#parameters-block-ga--runtime-182)); auto-translation is not yet implemented (deferred — audit finding 13.2) |
```
