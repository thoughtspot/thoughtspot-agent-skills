...
| `rely: { at_most_one_match }` | **Mapped** | → `cardinality: MANY_TO_ONE` on the TS join (which the platform *requires* — "both  type and cardinality should be defined", live-probed 2026-07-31). The reverse leg maps it back to `rely:` and writes no MV `cardinality:` key, so a round trip keeps the 17.3+ Runtime floor (BL-174) |
