...
| `properties.currency_type` | `format: { type: currency }` | v1.1 supports `format:` on measures, and (corrected 2026-07-29, finding 13.10) also on `fields:`/`dimensions:` entries — not measures-only |
...
| `format:` (currency/percentage) — measures OR fields/dimensions (corrected 2026-07-29, finding 13.10) | **Partial** | `currency_type` maps for measures; a formatted dimension's `format:` should be tolerated and logged, or mapped to the ThoughtSpot ATTRIBUTE `format_pattern` where a clean translation exists (not yet auto-emitted); percentage formatting has no TS equivalent |
