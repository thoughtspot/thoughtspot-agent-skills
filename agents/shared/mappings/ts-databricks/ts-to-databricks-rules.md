...
neither is present, and `rely:` works on every Databricks Runtime while `cardinality:`
requires **18.1+** (`databricks-metric-view.md:392`, `:436-437`). Emitting both — which
this converter did until BL-174 — silently raised a round-tripped MV's Runtime floor from
17.3+ to 18.1+ for no semantic gain. Conversely a `ONE_TO_MANY` join must **not** carry
