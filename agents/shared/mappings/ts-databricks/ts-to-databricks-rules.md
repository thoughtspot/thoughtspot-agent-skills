...
Is the formula a simple boolean expression?
  ...
  NO  → Contains parameters, LOD, or untranslatable functions?
        YES → Omit — log in Unmapped Report
             (Parameters: MV `parameters:` is a GA target (Runtime 18.2+), but this
             skill does not yet auto-emit it — omission here is "pending
             auto-translation," not "inherently untranslatable." See
             ../../schemas/databricks-metric-view.md#parameters-block-ga--runtime-182
             and audit finding 13.2, deferred.)
```
...
**Truncate the order field, not a transformed expression over it.** Databricks
requires a date-hierarchy level to sit on the order *field*; if the level is built
over a transformed date while the window orders by that transformed field, the
numbers are silently wrong (mislabelled buckets, missing period rows) while the MV
still imports and queries cleanly. Live-verified 2026-08-26 — see
[databricks-metric-view.md](../../schemas/databricks-metric-view.md)
"Date-Hierarchy Levels". Emission stays safe by requiring the window's date
argument to be a direct `[TABLE::COL]` reference and rejecting anything else as
untranslatable; do not relax that guard without re-reading that section.
...
| `joins[].type` (any of the four) | Dropped — an MV has no join-type field; Databricks fixes star-schema joins as `LEFT OUTER` ([Joins in metric views](https://docs.databricks.com/aws/en/business-semantics/metric-views/joins)). A source `LEFT_OUTER` is therefore lossless; an `INNER`/`RIGHT_OUTER`/`OUTER` model join is **not** reproducible and its row-retention semantics change |
| `joins[].cardinality: MANY_TO_ONE` or `ONE_TO_ONE` | `rely: { at_most_one_match: true }` — **and no `cardinality:` key** |
| `joins[].cardinality: ONE_TO_MANY` (or `MANY_TO_MANY`) | `cardinality: one_to_many` — **and no `rely:` block** |
| `joins[].cardinality` absent (Scenario A `referencing_join`) | `rely: { at_most_one_match: true }` (matches the MV's own `many_to_one` default) |

**Why never both** (corrected 2026-07-31, BL-174). `rely: { at_most_one_match: true }` and
`cardinality: many_to_one` are equivalent, `many_to_one` is the MV schema's default when
neither is present, so the second key adds nothing semantically. Emitting both — which
this converter did until BL-174 — was therefore pointless redundancy.

**Corrected 2026-08-26 (finding 13.13):** the original rationale here was that `rely:`
"works on every Databricks Runtime" while `cardinality:` requires 18.1+, so emitting both
raised the floor. That is not supported — Databricks' feature-availability matrix lists
"Join optimization with `rely.at_most_one_match`" under **18.1**, while its YAML reference
gates only one-to-many joins. Both readings are documented in
[databricks-metric-view.md](../../schemas/databricks-metric-view.md) "The `rely` Runtime
Floor"; neither is falsifiable on our test workspace. The **decision** stands (emitting
`rely:` alone is never worse than emitting both), but do not claim a `MANY_TO_ONE` join
keeps an MV at 17.3+. Conversely a `ONE_TO_MANY` join must **not** carry
`rely:`, which asserts the opposite of what the model declares.
