---
name: thoughtspot-model-builder
...
| [~/.claude/skills/thoughtspot-setup/SKILL.md](~/.claude/skills/thoughtspot-setup/SKILL.md) | ThoughtSpot auth, profile config, token persistence (Pattern A), API call patterns |
| [~/.claude/skills/snowflake-setup/SKILL.md](~/.claude/skills/snowflake-setup/SKILL.md) | Snowflake connection patterns, SQL execution, SHOW commands |

---

## Prerequisites

- ThoughtSpot profile configured — run `/thoughtspot-setup` if not
- Snowflake profile configured — run `/snowflake-setup` if not (required for Snowflake source; not needed for image source)
...
Authenticate ThoughtSpot using Pattern A from `thoughtspot-setup/SKILL.md`. Token written to `/tmp/ts_token.txt`.
